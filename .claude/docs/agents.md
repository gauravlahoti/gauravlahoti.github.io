...
Three **independent** Google ADK (agents-cli) projects. Atlas and Pulse each deploy their own Cloud Run service and keep `App(name="app")` + `agent_directory: app`; the Cloud Run service name comes from the `gcloud run deploy <name>` arg in each Makefile. Pulse stays at `min-instances=0` (its only callers are two Cloud Scheduler jobs, so a cold start on those is invisible); Atlas is `min-instances=1` — see its own entry below for why. RAG Lab is a standalone teaching agent served off-repo.

- **`agents/atlas/`** — **Atlas**, the chat-widget agent (service `atlas`). Seven live retrieval tools (`get_profile`, `get_work_history`, `get_projects`, `get_recent_posts`, `get_certifications`, `get_live_agents`, `get_site_stats`) plus two async action tools (`send_resume`, `send_note_to_gaurav`), all registered at agent level. Retrieval tools read content **live** via `app/corpus_live.py` (fetches `gauravlahoti.dev/content/*.json` with a short TTL; `app/corpus/*.json` is the offline fallback), so content edits — profile, work, projects, posts, certs, agents — reflect with NO redeploy. `get_site_stats` fetches the live question count from the resume-gate Worker. (Spec 37's frozen ADK Skills were retired in favor of live tools.) Routes: `POST /api/agent-chat` (SSE), `POST /api/agent-transcribe` (speech-to-text for the mic button, spec 48), `POST /api/agent-speak` (text-to-speech for spoken replies, spec 49), `GET /api/agent-chat/warm`, `GET /healthz`. Each has its own rate-limit bucket in `app/rate_limit.py` — `chat` 10/24h, `voice` 12/24h, `speak` 60/24h, per session and per IP-hash — so transcribing or speaking never spends a chat question. Voice models are pinned to Vertex on `adk-mas-demo` (not the ambient `GEMINI_API_KEY`): `gemini-3.5-transcribe-preview` → `gemini-3.7-flash` for STT, `gemini-3.1-flash-tts-preview` → `gemini-2.5-flash-tts` for TTS. Frontend: `assets/js/agent-widget.js`, lazy-loaded via `requestIdleCallback`, which in turn lazy-loads `agent-voice.js` (mic) and `agent-speech.js` (playback) on first use. Deployed at `--memory 512Mi --min-instances 1 --max-instances 5`. Spec 51 originally shipped `min-instances=0` plus a `portfolio-atlas-keepwarm` Cloud Scheduler job (pings `/api/agent-chat/warm` every 5 min) to dodge the ~18s cold start at effectively no cost — but named `min-instances=1` at 512Mi (~$8.70/month) as the explicit fallback if keep-warm proved unreliable. Cold starts kept recurring in practice, so Atlas now runs that fallback; a real instance is always up, so cold start is structurally impossible rather than merely unlikely. The keep-warm scheduler job is harmless but redundant now (nothing left for it to warm) — left running rather than torn out, since disabling it is a separate, easily-reversible decision. This also means the in-process rate-limit buckets effectively never reset from scale-to-zero.
