...
| WebMCP | `assets/js/webmcp.js` | Registers this site's WebMCP tools (`document.modelContext`, feature-detected against `navigator.modelContext` for older Chrome builds) so an AI agent can call named, typed functions instead of scraping the DOM. Pure `defineTools(ctx)` returns 5 tool defs — `search_site`, `get_profile` (section enum: overview/experience/certifications/resume), `list_work` (kind enum: projects/skills/domains/posts/agents/labs), `go_to` (scope-aware target enum; merges scroll, chat-open, and lab navigation), and human-in-the-loop `draft_note_to_gaurav` that only pre-fills the Atlas composer and never sends — consolidated from an earlier 13-def/11-tool registry to avoid tool overlap (Chrome's WebMCP best practices explicitly warn against it) and match verified real-world adopters, which cluster at 1-4 hand-authored tools. `registerWebMcp({scope, profile})` filters by page scope and registers with the browser. No write tools. `main.js` and each standalone page module call `registerWebMcp` with their own scope on boot; see `.claude/specs/45-webmcp-agent-ready.md` for the full scope table and consolidation rationale. |
